<script>
  ...
  // 机构基本信息
  let institutionInfo = {
    org1: { name: "", type: "", description: "" },
    org2: { name: "", type: "", description: "" }
  };

  // ES接口地址
  const ES_URL = "http://81.70.12.153:9388/patents/_search";
    ...
    try {
      ...
      const suggestions = (data.aggregations?.applicants?.buckets || []).map(b => ({ name: b.key, id: b.key }));
      if (which === 1) org1Suggestions = suggestions;
      if (which === 2) org2Suggestions = suggestions;
    } catch (e) {
      if (which === 1) org1Suggestions = [];
      if (which === 2) org2Suggestions = [];
    }
    ...
    if (which === 1) {
      ...
      institutionInfo = { ...institutionInfo }; // 触发更新
  ...
  async function validateOrg(which) {
    if (which === 1) {
      ...
      if (found) {
        org1 = found.name;
        institutionInfo.org1.name = found.name;
        institutionInfo.org1.type = "待确定";
        institutionInfo.org1.description = "简介加载中...";
        institutionInfo = { ...institutionInfo };
        institutionInfo.org1.description = await fetchWikiSummary(found.name);
        institutionInfo = { ...institutionInfo };
      } else {
        org1Input = "";
        org1 = "";
    ...
    if (which === 2) {
      ...
      if (found) {
        org2 = found.name;
        institutionInfo.org2.name = found.name;
        institutionInfo.org2.type = "待确定";
        institutionInfo.org2.description = "简介加载中...";
        institutionInfo = { ...institutionInfo };
        institutionInfo.org2.description = await fetchWikiSummary(found.name);
        institutionInfo = { ...institutionInfo };
      } else {
        org2Input = "";
        org2 = "";
  ...
  async function handleSearch() {
    ...
    try {
      const [desc1, desc2] = await Promise.all([
        fetchWikiSummary(org1),
        fetchWikiSummary(org2)
      ]);
      institutionInfo = {
        org1: { name: org1, type: "待确定", description: desc1 },
        org2: { name: org2, type: "待确定", description: desc2 }
      };
    } catch (error) {
  ...
  async function handleAnalysis() {
    ...
    try {
      ...
      const prompt = `请对如下两个机构进行专利机构对比分析：${JSON.stringify(data, null, 2)}`;
      generateReportWithPrompt(prompt, "专利机构对比主题轨迹报告");
    } catch (error) {
  ...
  function showCooperate() { dispatch('showCooperate'); }
  function showPatentComparison() { dispatch('showPatentComparison'); }
  function showPaperComparison() { dispatch('showPaperComparison'); }
...
<div class="panel-inner bg-white dark:bg-gray-800 rounded overflow-hidden shadow-lg w-full h-full">
  ...
  <div class="flex justify-between items-center p-3 border-b border-gray-200 dark:border-gray-700">
    ...
    <div class="flex items-center">
     ...
     on:mouseenter={() => document.getElementById('comparisonDownloadMenu').classList.remove('hidden')}
     on:mouseleave={() => {
        setTimeout(() => {
          const menu = document.getElementById('comparisonDownloadMenu');
  ...
  <div id="comparisonDownloadMenu" 
       class="download-options absolute hidden bg-white dark:bg-gray-700 shadow-lg rounded py-2 z-50 w-36"
       on:mouseenter={() => document.getElementById('comparisonDownloadMenu').classList.remove('hidden')}
       on:mouseleave={() => document.getElementById('comparisonDownloadMenu').classList.add('hidden')}>
    ...
    <div class="flex items-center">
      ...
      <div class="flex items-center space-x-6 mr-6">
        <div class="relative"
     on:mouseenter={() => document.getElementById('patentComparisonMenu').classList.remove('hidden')}
     on:mouseleave={() => {
        setTimeout(() => {
          const menu = document.getElementById('patentComparisonMenu');
          const isHovered = menu.matches(':hover');
          if (!isHovered) {
            menu.classList.add('hidden');
          }
     ...
     on:focus={() => document.getElementById('patentComparisonMenu').classList.remove('hidden')}>
  <button class="text-primary font-medium hover:text-primary/80 flex items-center">
    <span>机构轨迹对比</span>
  </button>
  <div id="patentComparisonMenu" 
       ...
       on:mouseenter={() => document.getElementById('patentComparisonMenu').classList.remove('hidden')}
       on:mouseleave={() => document.getElementById('patentComparisonMenu').classList.add('hidden')}>
  ...
  <div class="content-area">
    <div class="homebody-container">
      <main class="container mx-auto px-4 py-8">
        <div class="mb-8">
          <h2 class="text-2xl font-bold text-gray-800 mb-2">专利机构对比分析</h2>
          <p class="text-gray-600">
            通过多维度数据对比，深入了解不同机构的专利发表情况与产业创造力
          </p>
        </div>
